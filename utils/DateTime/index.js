...
{
    const { delimiter = ' - ', monthType = 'normal', timeLang = 'th', monthLang = 'th', yearLang = 'en', yearType = 'normal' } = options
    ...
    return getDateString(date, { monthType, monthLang, yearLang, yearType }) + delimiter + getTimeString(date, timeLang);
}

function getDateString(dateOrTime, options = {})
{
    const { monthType = 'normal', monthLang = 'th', yearLang = 'en', yearType = 'normal' } = options
    ...
    const year = date.getFullYear() + yearOffsets[yearLang];

    let yyyy = yearType === 'none' ? '' : ' ' + year

    if (monthType === 'short')
    {
        return day + ' ' + monthMins[monthLang][month] + yyyy;
    }
    ...
    return day + ' ' + months[monthLang][month] + ' ' + yyyy;
}
